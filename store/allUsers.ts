...
const allUsersSlice = createSlice({
  ...
  reducers: {
    ...
    setFriendPending:(state,action:PayloadAction<{userid:string}>) =>{

      const cur_user = state.users.find((user)=>user.user_id === action.payload.userid)

      if (cur_user){
        cur_user.isFriend = "pending"
      }

    },

    setFriendAccepted:(state,action:PayloadAction<{userid:string}>) =>{

      const cur_user = state.users.find((user)=>user.user_id === action.payload.userid)

      if (cur_user){

        cur_user.isFriend = "accepted"
      }


    },

    setFriendRejected: (state,action:PayloadAction<{userid:string}>)=>{

      const cur_user = state.users.find((user)=>user.user_id === action.payload.userid)

      if (cur_user){

        cur_user.isFriend = "rejected"
      }
...
export const {setFriendPending,setFriendAccepted,setFriendRejected} = allUsersSlice.actions;
